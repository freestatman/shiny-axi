...
// Address the server binds to (SHINY_AXI_HOST). Defaults to loopback. A wildcard value
// (0.0.0.0 or ::) binds every interface.
export function bindHost(env = process.env) {
  return env.SHINY_AXI_HOST?.trim() || LOOPBACK_HOST;
}
...
// Hostname written into the session URLs the server generates (SHINY_AXI_LINK_HOST).
// Defaults to the host the CLI dials.
export function linkHost(env = process.env) {
  return env.SHINY_AXI_LINK_HOST?.trim() || clientHost(env);
}
...
export function stateDir() {
  return process.env.SHINY_AXI_STATE_DIR || path.join(os.homedir(), ".shiny-axi");
}
...
export function defaultPort() {
  return Number(process.env.SHINY_AXI_PORT || 4387);
}
