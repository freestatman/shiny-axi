...
const DEFAULT_TITLE = "Shiny AXI CLI";
const DEFAULT_REQUEST_TIMEOUT_MS = 1_000;

export function resolveTelemetryConfig(input) {
  const optOut = String(input.env.SHINY_AXI_TELEMETRY || "")
    .trim()
    .toLowerCase();
  ...
  const websiteID = String(input.env.SHINY_AXI_UMAMI_WEBSITE_ID || "").trim() || input.buildWebsiteID.trim();
  if (!websiteID) {
    return { enabled: false, host: "", websiteID: "" };
  }

  const host = String(input.env.SHINY_AXI_UMAMI_HOST || "").trim() || input.buildHost.trim() || HARDCODED_FALLBACK_HOST;
  return { enabled: true, host, websiteID };
}

export function getBuildTimeUmamiHost() {
  return process.env.SHINY_AXI_BUILD_UMAMI_HOST || "";
}

export function getBuildTimeUmamiWebsiteID() {
  return process.env.SHINY_AXI_BUILD_UMAMI_WEBSITE_ID || "";
}
