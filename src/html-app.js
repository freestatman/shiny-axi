...
// (https://ht-ml.app), a third-party hosting service not part of Shiny AXI, and return a visitable
...
export function htmlAppApiUrl(env = process.env) {
  return String(env.SHINY_AXI_HTML_APP_API_URL || DEFAULT_API_URL).replace(/\/+$/, "");
}
 ...
 * @param {string} [options.apiUrl] Override the API base (defaults to SHINY_AXI_HTML_APP_API_URL or ht-ml.app).
...
export async function publishToHtmlApp(html, options = {}) {
  ...
  const token = optionalString(options.token ?? env.SHINY_AXI_HTML_APP_TOKEN);

  const headers = { "content-type": "application/json", "user-agent": "shiny-axi" };
