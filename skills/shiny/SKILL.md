...
2. **Poll for annotations**: Run `npx -y shiny-axi poll <app-dir>`. The poll stays silent and waits for user annotations - leave it running, never kill it.
...
3. **Poll for annotations**: Run `npx -y shiny-axi poll <app-dir>`. The poll stays silent and waits for user annotations - leave it running, never kill it.
...
7. **Reply & Wait**: Run `npx -y shiny-axi poll <app-dir> --agent-reply "Applied the changes!"` to show your message in the browser and wait for further annotations.
8. **End session**: Run `npx -y shiny-axi end <app-dir>` when the review session is complete.
