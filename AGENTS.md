...
The CLI is built on `axi-sdk-js` (`runAxiCli`). The `home()` callback returns the rich object shown when the user runs `shiny-axi` with no arguments - this is the same TOON-serialized output that lands in the agent's optional `SessionStart` hook after `shiny-axi setup hooks` (`sessions`, `visual_guidance`, `playbooks`, `help`). Top-level `--help` returns the same static guidance without dynamic sessions, `shiny-axi playbook [playbook_id]` exposes focused artifact guidance, and `shiny-axi design` prints copy-pasteable Tailwind/DaisyUI CDN URLs plus the DaisyUI component reference as a fallback after user-specified and subject-project design sources come up empty. That design output recommends `data-theme="luxury"` as the default DaisyUI theme, warns that `@apply`ing DaisyUI classes inside `<style type="text/tailwindcss">` aborts the Tailwind browser-runtime compile, and includes an optional copy-paste layout safety CSS snippet that Shiny AXI does not auto-inject. `src/skill.js` renders the installable Agent Skill from the same home output, rewriting command examples to non-interactive `npx -y shiny-axi ...` invocations, omitting live session state, and including Hermes Agent frontmatter metadata for categorization. The generated skill intentionally omits a `version` frontmatter field because release-please updates `package.json` without regenerating `skills/shiny-axi/SKILL.md`. The playbook guidance tells agents that one artifact can combine several playbooks and to read every relevant playbook for best quality. The bare-arg form (`shiny-axi some.html`) is normalized into `["open", "some.html"]` by `normalizeArgv`.
