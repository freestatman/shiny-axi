...
test("htmlAppApiUrl defaults to ht-ml.app and honors the override env", () => {
  ...
  assert.equal(htmlAppApiUrl({ SHINY_AXI_HTML_APP_API_URL: "http://127.0.0.1:9/" }), "http://127.0.0.1:9");
});
...
test("publishToHtmlApp sends a bearer token when one is configured", async () => {
  ...
  await publishToHtmlApp("<h1>Hi</h1>", { fetch: fetchImpl, env: { SHINY_AXI_HTML_APP_TOKEN: "tok_123" } });
