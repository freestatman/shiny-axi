...
test("package metadata matches the GitHub repository used for npm provenance", async () => {
  ...
  assert.equal(packageJson.repository.url, "git+https://github.com/freestatman/shiny-axi.git");
  assert.equal(packageJson.bugs.url, "https://github.com/freestatman/shiny-axi/issues");
  assert.equal(packageJson.homepage, "https://github.com/freestatman/shiny-axi#readme");
});
